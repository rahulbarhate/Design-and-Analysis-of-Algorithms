...
void insert(hterm element)
{
    ...
    while (list[now / 2].cost > element.cost) {//check if parent cost is greater than element cost 
...
hterm delete()//for returning min element and deleting it from heap 
{
	    hterm minElement, lastElement;
		int now,child;

    	minElement = list[1];//first element of the list will be minimum
	...
	for (now = 1; now * 2 <= heap_size; now = child)
	 {
		child = now * 2;
		if (child != heap_size && list[child + 1].cost < list[child].cost)//now k right child ka cost <now k left child ka cost
		 {

            		child++;//right node will be child node

        	}//i.e.to find min from heap
		if (lastElement.cost > list[child].cost) //to
		{
...
int rowReduction(int red_mat[][10],int row[],int v)
{
	...
	for(i=1;i<=v;i++)
	{
		for(j=1;j<=v;j++)
		{
			if(red_mat[i][j]<row[i])//for finding min in row
			{
				row[i]=red_mat[i][j];
			}
		}
	...
	for(i=1;i<=v;i++)
	{
		for(j=1;j<=v;j++)
		{
			...
			{
				red_mat[i][j]-=row[i];//subtract min row value from matrix
			}
		}
	}
}
...
int  reduce_matrix(int red_mat[][10])
{
	...
	return cost;//return lower bound of matrix
}
...
hterm expand(hterm node)
{
	...
	while(1)
	{
		...
		if(k==v) break;//total no of vertices=node.path[0]+1(means all nodes included)
		
		node=delete();//return min cost node
	}
	return node;
}
